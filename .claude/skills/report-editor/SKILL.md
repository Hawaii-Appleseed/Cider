...
| `setStyle(key, patch)` | typography for a slot key or `'text.N'`: `{size, font, weight, color, tracking, leading, italic, underline, align, case, before, after, indent, smallcaps, hyphens}` (`before`/`after` = paragraph space in px; `indent` = first-line px, negative hangs); `null` clears one. `{style: 'Body'}` makes the text WEAR a named style (replacing its local formatting; other keys in the same patch ride on top as overrides); `{style: null}` detaches, keeping the look inline |
