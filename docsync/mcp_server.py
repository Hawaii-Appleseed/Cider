...
TOOLS = [
    ...
    {"name": "pilot", "fn": t_pilot,
     "description": "CHANGE a report: run one window.docsync.api verb in the open "
                    ...
                    "setSlot, setStyle, setBoxText, setChart, place, recolor, rotate, lock, "
                    ...
                    "stylesheets with how many things wear each, defineStyle "
                    "creates or redefines one, dress puts an object style on a "
                    "shape/box/table and setStyle({use:'Body'}) puts a text style "
                    "on a slot. Prefer those over setting the same keys on each "
                    "object by hand. setChart(id, patch) changes a chart as the "
                    "Chart panel would — patch.points {seriesName: n} sets its "
                    "numbers by name. Geometry is in "
